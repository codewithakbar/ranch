...
from .models import Bannercha, Book, Category, ContentTable, Images, MalImages, Malumotlar, Content, Fakultetlar, OqishniKochirish, Rektorat, HomeSlider
...
def category(request, cat_id=None):
    ...
    if catID:
        yangiliklar = Yangiliklar.objects.filter(base_category__id=catID).order_by("-id")
    else:
    ...
    table = ContentTable.objects.filter(content__category__id=catID)
    ...
    context = {
        ...
        "con_table": ContentTable.objects.filter(content__category__id=catID).first(),
